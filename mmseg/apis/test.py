...
def single_gpu_test(model,
    ...
    for i, data in enumerate(data_loader):
        with torch.no_grad():
            # Some CCD val pipelines include gt_semantic_seg for PR; drop for test/inference
        ...
        if show or out_dir:
            ...
            # Handle 6-channel cross-modal inputs by visualizing T2 (last 3 channels)
            if img_tensor.dim() == 4 and img_tensor.size(1) == 6:
                img_tensor = img_tensor[:, 3:6, :, :]
            imgs = tensor2imgs(img_tensor, **img_metas[0]['img_norm_cfg'])
            assert len(imgs) == len(img_metas)

            # Unwrap result for visualization (some models return list of dicts)
            res_vis = result
            ...
            for img, img_meta in zip(imgs, img_metas):
                ...
                if out_dir:
                    rel_name = img_meta['ori_filename']
                    if osp.isabs(rel_name):
                        rel_name = osp.basename(rel_name)
                    out_file = osp.join(out_dir, rel_name)
                    mmcv.mkdir_or_exist(osp.dirname(out_file))
                else:
                    out_file = None

                # Cross-modal models return dicts; handle visualization manually
                if isinstance(res_vis, dict) and out_dir:
                    import numpy as _np
